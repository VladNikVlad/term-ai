...
export default function Navbar({scrolled}) {
    ...
    return (
        ...
        "bg-slate-920/80 backdrop-blur-lg  border-b border-slate-800": 
        "bg-slate-920/20 backdrop-blur-sm"}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-14 sm:h-16 md:h-20">
                    ...
                    <button className="md:hidden p-2 text-gray-300 hover:text-white "
                        ...
                        {mobileMenuOpen ? (<X className="w-5 h-6 sm:w-6 sm:h-6" />) :
                            <Menu className="w-5 h-6 sm:w-6 sm:h-6" />
                        }
                    </button>
                </div>
            </div>
            {mobileMenuOpen && (
                <div className="md:hidden bg-slate-900/95 backdrop-blur-ld border-t border slate-800 animate-in slide-in-from-top duration-300">
